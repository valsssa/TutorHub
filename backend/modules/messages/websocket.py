...
class WebSocketManager:
    ...
    async def send_personal_message(self, message: dict, user_id: int) -> bool:
        ...
        if success_count > 0:
            logger.debug(
                f"📤 Delivered to user {user_id}: "
                f"success={success_count}, failed={len(disconnected)}"
            )
            return True
        else:
...
async def authenticate_websocket(
    websocket: WebSocket, token: str, db: Session
) -> Optional[User]:
    """
    ...
    try:
        ...
        while True:
            try:
                ...
                if not event_type:
                    await websocket.send_json(
                        {"type": "error", "message": "Missing event type"}
                    )
                    continue
                ...
                if event_type == "ping":
                    ...
                    if isinstance(user_ids, list):
                        ...
                        offline_users = [
                            uid for uid in user_ids if uid not in online_users
                        ]
                        await websocket.send_json(
                            {
                                "type": "presence_status",
