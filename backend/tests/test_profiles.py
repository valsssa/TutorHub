...
class TestGetProfile:
    ...
    def test_get_profile_creates_if_not_exists(
        ...
        profile = (
            ...
            .filter(UserProfile.user_id == student_user.id)
            .first()
        )
        if profile:
        ...
        response = client.get(
            "/api/profile/me", headers={"Authorization": f"Bearer {student_token}"}
        )
...
class TestUpdateProfile:
    ...
    def test_update_existing_profile(
        ...
        profile = UserProfile(
            user_id=student_user.id, full_name="Original Name", city="Original City"
        )
        db_session.add(profile)
        db_session.commit()
