                    ...
                    <>
                        ...
                        <div className="flex-1 overflow-auto bg-white dark:bg-slate-900 relative">
                            ...
                            <div className={view === 'Day' ? "w-full" : "min-w-[600px] md:min-w-[800px]"}>
                                {/* Header Row */}
                                <div className={`grid ${view === 'Day' ? 'grid-cols-[50px_1fr]' : `grid-cols-[60px_repeat(${days.length},1fr)]`} border-b border-slate-200 dark:border-slate-800 sticky top-0 bg-white dark:bg-slate-900 z-30`}>
                                ...
                                <div className="relative">
                                    {hours.map((time, i) => (
                                        <div key={i} className={`grid ${view === 'Day' ? 'grid-cols-[50px_1fr]' : `grid-cols-[60px_repeat(${days.length},1fr)]`} h-20 sm:h-24 border-b border-slate-100 dark:border-slate-800`}>
                                            {/* Sticky Time Label */}
                    ...
                    <div className="flex flex-col h-full animate-in fade-in slide-in-from-right-4 duration-300">
                        ...
                        <div className="flex-1 overflow-y-auto p-3 sm:p-6 space-y-4">
                            {weeklySchedule.map((day, dayIndex) => (
                                <div key={day.day} className={`p-4 rounded-xl border transition-all ${day.enabled ? 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700 shadow-sm' : 'bg-slate-50 dark:bg-slate-900 border-transparent opacity-60'}`}>
                                    <div className="flex flex-col sm:flex-row sm:items-start gap-4">
                                        {/* Header Row: Toggle + Name + Actions */}
                                        <div className="flex items-center justify-between w-full sm:w-auto sm:min-w-[120px]">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <h3 className={`font-bold text-base ${day.enabled ? 'text-slate-900 dark:text-white' : 'text-slate-500'}`}>{day.day}</h3>
                                            </div>
                                            
                                            {/* Mobile Actions (visible only when enabled) */}
                                            <div className="sm:hidden flex gap-1">
                                                 {day.enabled && (
                                                    <>
                                                        <button onClick={() => addSlot(dayIndex)} className="p-2 text-emerald-600 bg-emerald-50 dark:bg-emerald-900/20 rounded-lg" title="Add time slot">
                                                            <Plus size={18} />
                                                        </button>
                                                        <button onClick={() => copyToAll(dayIndex)} className="p-2 text-slate-500 bg-slate-100 dark:bg-slate-800 rounded-lg" title="Copy schedule to all days">
                                                            <Copy size={18} />
                                                        </button>
                                                    </>
                                                )}
                                        ...
                                        <div className="flex-1 w-full pl-0 sm:pl-4 sm:border-l sm:border-slate-100 sm:dark:border-slate-800">
                                            ...
                                            <div className="hidden sm:flex justify-end gap-2 mb-3">
                                                 {day.enabled && (
                                                    <>
                                                        <button onClick={() => addSlot(dayIndex)} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-emerald-700 bg-emerald-50 dark:bg-emerald-900/20 dark:text-emerald-400 rounded-lg hover:bg-emerald-100 dark:hover:bg-emerald-900/40 transition-colors">
                                                            <Plus size={14} /> Add Slot
                                                        </button>
                                                        <button onClick={() => copyToAll(dayIndex)} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-slate-600 bg-slate-100 dark:bg-slate-800 dark:text-slate-400 rounded-lg hover:bg-slate-200 transition-colors">
                                            ...
                                            {day.enabled ? (
                                                <div className="space-y-3">
                                                    {day.slots.map((slot, slotIndex) => (
                                                        <div key={slotIndex} className="flex items-center gap-2 animate-in fade-in slide-in-from-top-1 duration-200">
                                                    ...
                                                    {day.slots.length === 0 && (
                                                        <div className="text-sm text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/20 px-4 py-3 rounded-lg border border-amber-100 dark:border-amber-800 flex items-center gap-2">
                                                            <Info size={16} /> No slots added. Add a slot to appear available.
                                                        </div>
                                                    )}
                                                </div>
                                            ) : (
                                                <div className="sm:h-full flex items-center">
                                                    <p className="text-sm text-slate-400 italic">Marked as unavailable</p>
                                                </div>
                                            )}
                                        </div>
                                    </div>
