...
import { Search, ChevronDown, SlidersHorizontal, X, Check, Sun, Moon, Sunrise, Sunset, Globe, Star, ArrowLeft } from 'lucide-react';
import { Tutor, FilterState } from '../domain/types';
import { SUBJECTS, SORT_OPTIONS } from '../domain/constants';
...
export const MarketplacePage: React.FC<MarketplacePageProps> = ({ 
  ...
  const filteredTutors = useMemo(() => {
      return tutors.filter(tutor => {
          // 1. Search Query (Name, Subject, Bio)
          if (filters.searchQuery) {
              ...
              const matchesKeyword = 
                  ...
                  (tutor.topics && tutor.topics.some(t => t.toLowerCase().includes(q)));
          ...
          if (filters.subject && filters.subject !== 'All') {
              // Partial match to handle "Math" vs "Mathematics"
              if (!tutor.subject.toLowerCase().includes(filters.subject.toLowerCase())) {
                  return false;
              }
          }
          ...
          if (filters.maxPrice < 200) {
              if (tutor.hourlyRate > filters.maxPrice) return false;
          }

          // 4. Native Speaker
          if (filters.nativeSpeaker) {
              if (filters.subject && LANGUAGES_LIST.includes(filters.subject)) {
                  if (tutor.nativeLanguage !== filters.subject) return false;
              } else if (tutor.nativeLanguage !== 'English') {
                  return false;
              }
          }
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      ...
      {/* Hero Header */}
      <div className="grid lg:grid-cols-2 gap-12 items-center mb-16 mt-4">
          {/* Left Content */}
          ...
          <div className="relative hidden lg:block h-[500px]">
              ...
              <div className="absolute -bottom-8 -left-8 bg-white dark:bg-slate-800 p-4 rounded-2xl shadow-xl border border-slate-100 dark:border-slate-700 flex items-center gap-4 animate-in slide-in-from-bottom-4 fade-in duration-700">
              ...
              <div className="absolute top-12 -right-8 bg-white dark:bg-slate-800 p-4 rounded-2xl shadow-xl border border-slate-100 dark:border-slate-700 animate-in slide-in-from-right-4 fade-in duration-700 delay-200">
      ...
      <div className="md:hidden sticky top-20 z-30 mb-6 bg-slate-50 dark:bg-slate-950 pb-2 -mx-4 px-4">
          <div className="flex gap-2">
              ...
              <button 
                ...
                className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-white px-4 rounded-xl shadow-sm flex items-center justify-center hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors"
              >
                  <SlidersHorizontal size={20} />
              </button>
          </div>
          {/* Active Filter Chips (Optional Preview) */}
      ...
      <div className="hidden md:block mb-8 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="relative z-20">
               ...
               <div className="relative z-10">
                  ...
                  {activeDropdown === 'country' && (
                      <>
                        <div className="fixed inset-0 z-10" onClick={() => setActiveDropdown(null)} />
                        <div className="absolute top-[calc(100%+8px)] left-0 w-full md:w-[300px] bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl shadow-xl z-30 animate-in fade-in zoom-in-95 duration-200 overflow-hidden">
              ...
              <div className="relative z-0">
                  ...
                  {activeDropdown === 'availability' && (
                      <>
                        <div className="fixed inset-0 z-10" onClick={() => setActiveDropdown(null)} />
                        <div className="absolute top-[calc(100%+8px)] right-0 left-0 md:left-auto w-full md:w-[320px] bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl shadow-xl z-30 animate-in fade-in zoom-in-95 duration-200 p-5 overflow-y-auto max-h-[500px]">
          ...
          {/* Row 2: Secondary Filters (Desktop) */}
          <div className="flex flex-col sm:flex-row gap-3">
              <button 
                onClick={() => onOpenFilter('Personalization')}
                className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold px-6 py-3 rounded-xl flex items-center justify-center gap-2 transition-colors shadow-lg shadow-emerald-500/20 active:scale-95 whitespace-nowrap"
              >
               ...
               <div className="relative">
                  ...
                  {activeDropdown === 'languages' && (
                      <>
                        <div className="fixed inset-0 z-10" onClick={() => setActiveDropdown(null)} />
                        <div className="absolute top-[calc(100%+8px)] left-0 right-0 sm:right-auto w-full sm:w-[280px] bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl shadow-xl z-30 animate-in fade-in zoom-in-95 duration-200 overflow-hidden">
                             {/* Search Input */}
                             <div className="p-3">
                                <div className="relative">
                                    <Search size={16} className="absolute left-3 top-2.5 text-slate-400" />
                ...
                <div className="relative">
                  ...
                  {activeDropdown === 'native' && (
                      <>
                        <div className="fixed inset-0 z-10" onClick={() => setActiveDropdown(null)} />
                        <div className="absolute top-[calc(100%+8px)] left-0 right-0 sm:right-auto w-full sm:w-[300px] bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl shadow-xl z-30 animate-in fade-in zoom-in-95 duration-200 p-5">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
          <h2 className="text-xl font-bold text-slate-900 dark:text-white">
              {sortedTutors.length.toLocaleString()} {filters.subject || 'Teachers'} available
          </h2>
          
          <div className="flex items-center gap-2 w-full sm:w-auto">
              <span className="text-sm text-slate-500 dark:text-slate-400 whitespace-nowrap">Sort by:</span>
              <div className="relative group flex-1 sm:flex-initial min-w-[180px]">
      ...
      {/* Mobile Filter Modal */}
      {isMobileFilterOpen && (
          <div className="fixed inset-0 z-50 bg-white dark:bg-slate-950 flex flex-col md:hidden animate-in slide-in-from-bottom-full duration-300">
              {/* Modal Header */}
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-8 pb-32">
                  ...
                  <div>
                      ...
                      <select 
                          ...
                          className="w-full p-3 bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl text-sm"
                      >
                  ...
                  {/* Country */}
                  <div>
                      <label className="block text-sm font-bold text-slate-900 dark:text-white mb-2">Tutor From</label>
                      <select 
                          value={filters.country}
                          onChange={(e) => handleInputChange('country', e.target.value)}
                          className="w-full p-3 bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl text-sm"
                      >
                          {DISPLAY_COUNTRIES.map(c => <option key={c} value={c}>{c}</option>)}
                      </select>
                  </div>

                  {/* Native Speaker */}
                  <div className="flex items-center justify-between p-4 border border-slate-200 dark:border-slate-800 rounded-xl">
                      <span className="text-sm font-bold text-slate-900 dark:text-white">Native Speaker Only</span>
                      <button 
                            onClick={() => handleInputChange('nativeSpeaker', !filters.nativeSpeaker)}
                            className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${filters.nativeSpeaker ? 'bg-emerald-500' : 'bg-slate-200 dark:bg-slate-700'}`}
                        >
                            <span className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${filters.nativeSpeaker ? 'translate-x-5' : 'translate-x-0'}`} />
                        </button>
                  </div>
              </div>
              ...
              <div className="p-4 border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 absolute bottom-0 left-0 w-full flex gap-3">
                  ...
                  >
                      Show {sortedTutors.length} Tutors
                  </button>
              </div>
          </div>
      )}
